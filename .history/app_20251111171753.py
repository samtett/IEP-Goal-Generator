...
def main():
    ...
    *Retrieval-Augmented Generation System for Special Education Transition Planning*
    ...
    llm_provider = st.sidebar.radio(
        ...
        help="Groq offers free access to powerful models like Llama 3. OpenAI requires credits."
    )
    ...
    if st.button("✨ Generate IEP Goals", type="primary", use_container_width=True):
        ...
        with st.spinner(f"Generating goals using {model_choice}... (this may take 30-60 seconds)"):
            try:
                # Select appropriate generator based on provider
                if llm_provider == "Groq (FREE)":
                    generator = GroqGenerator(api_key=api_key, model=model_choice)
                else:
    ...
    if 'last_generated' in st.session_state:
        ...
        if st.button("Refine Goals") and feedback:
            if not api_key:
                st.error(f"Please provide a {'Groq' if llm_provider == 'Groq (FREE)' else 'OpenAI'} API key.")
            else:
